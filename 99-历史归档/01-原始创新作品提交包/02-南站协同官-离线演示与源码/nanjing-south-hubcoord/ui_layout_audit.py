...
def assert_layout(page, path: str, width: int) -> None:
    metrics = page.evaluate(
        ...
        () => {
          ...
          const lockedLines = [...document.querySelectorAll('.cover-title-line, .hero-title-line, .heading-line, .cockpit-brief .hero-line')]
            .map((el) => ({
          ...
          for (const el of document.querySelectorAll('h1, h2, h3')) {
            if (el.querySelector('.cover-title-line, .hero-title-line, .heading-line, .cockpit-brief .hero-line') || el.querySelector('br')) continue;
...
with sync_playwright() as playwright:
    ...
    for width, height in VIEWPORTS:
        ...
        for path in PAGES:
            ...
            assert logo.evaluate("el => el.complete && el.naturalWidth > 0"), (path, width, "brand mark failed to load")
