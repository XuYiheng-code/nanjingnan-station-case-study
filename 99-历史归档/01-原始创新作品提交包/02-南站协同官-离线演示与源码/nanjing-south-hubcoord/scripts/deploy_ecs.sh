...
for path in story.html analysis.html showcase.css showcase.js assets/nanjing-south-aerial.jpeg agent-home.html case.html mechanism.html solution.html demo.html evidence.html cad/ cad/data/facts.json; do
  curl -fsS -o /dev/null "https://hubcoord.cn/$path"
done

echo "[7/7] 清理远端旧备份（保留最近三份）"
ssh "$SSH_TARGET" "set -eu
