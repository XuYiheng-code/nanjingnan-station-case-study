...
这是面向评委专家的六页展示站，也是一个可操作的案例教学与模拟推演原型。阅读路径为“认识协同官—案例全景—南站破局—智能体系统—操作 Demo—证据与验证”，并配有三维治理沙盘和影像作品页。工作台读取已审阅的展示证据、固定权责规则和 SC-01 教学情景，将事件线索整理为需要规定角色确认的协同议题卡。

线上展示地址为 `https://hubcoord.cn`，打开后直接进入网站，无需登录。后续更新可直接执行 `./scripts/deploy_ecs.sh`。服务器和证书配置见 `deploy/DEPLOY.md`。
...
```bash
cd '02-南站协同官-离线演示与源码/nanjing-south-hubcoord'
python3 -m venv .venv
