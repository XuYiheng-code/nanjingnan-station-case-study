...
const presentationSteps = [
  { selector: "#top", title: "先交代边界", note: "这是案例教学模拟。系统不接真实业务数据，也不替任何主体作出处置决定。", button: "查看事件传播" },
  { selector: ".scenario-timeline", title: "看见跨界传播", note: "晚点压力从铁路运行域传到停车运营域和外围道路，旅客路径连续，治理责任分段。", button: "调取已审阅证据" },
  ...
  { selector: "#coordinationCard", title: "现场主管先确认", note: "五个智能体已经给出最小证据集和判断链。点击后记录现场主管的模拟确认理由。", button: "记录现场主管确认", action: "confirm_operator" },
  { selector: "#coordinationCard", title: "综管办再确认", note: "跨主体事项不会因一个角色同意而生效。现在切换综管办值守人员，记录第二次模拟确认。", button: "记录综管办确认", action: "confirm_management" },
  { selector: "#coordinationCard", title: "再试一次越权请求", note: "把动作改成直接行政处置。安全审计 Agent 应当拦截确认，只允许拒绝或升级。", button: "运行越权对照", action: "test_restricted" },
  { selector: "#coordinationCard", title: "边界已被实际拦截", note: "确认按钮已经禁用。演示到这里完成：系统能协同，但不能借智能化改写法定权责。", button: "回到开头", action: "restart" }
];
...
async function runAgentPipeline() {
  ...
  for (const item of items) {
    ...
    if (!reduced) await new Promise((resolve) => setTimeout(resolve, 135));
...
function updatePresentationGuide(scroll = true) {
  ...
  if (scroll) document.querySelector(step.selector)?.scrollIntoView({ behavior: "smooth", block: "start" });
}
