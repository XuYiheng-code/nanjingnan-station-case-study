...
PROJECT_FILES = (
    ...
    "static/showcase.js",
    "static/styles.css",
    "static/assets/films/hubcoord-agent-60s-poster.webp",
    "static/assets/films/hubcoord-agent-60s.mp4",
    "static/assets/films/hubcoord-agent-60s-nobgm.mp4",
    "static/assets/films/hubcoord-agent-60s.zh-CN.vtt",
    "ui_layout_audit.py",
    "ui_offline_check.py",
    "南站协同官-直接打开.html",
    "南站协同官-工作台Demo.html",
...
def check_environment_and_ports(report: Report) -> None:
    ...
    if not positive_integer(effective["QWEN_TIMEOUT_SECONDS"], maximum=60):
        invalid.append("QWEN_TIMEOUT_SECONDS 必须是 1–60 的整数")
    if not positive_integer(effective["QWEN_MAX_TOKENS"], maximum=1600) or int(effective["QWEN_MAX_TOKENS"]) < 128:
        invalid.append("QWEN_MAX_TOKENS 必须是 128–1600 的整数")
    if not positive_integer(effective["ASSISTANT_RATE_LIMIT_PER_MINUTE"], maximum=60):
        invalid.append("ASSISTANT_RATE_LIMIT_PER_MINUTE 必须是 1–60 的整数")
    if effective["QWEN_BASE_URL"] != "https://dashscope.aliyuncs.com/compatible-mode/v1":
        invalid.append("QWEN_BASE_URL 必须使用阿里云百炼兼容接口")
    try:
    ...
    if not effective.get("DASHSCOPE_API_KEY", ""):
        report.warn("DASHSCOPE_API_KEY 未配置；静态站和确定性演示可用，小南模型问答不可用")
...
def check_static_assets(report: Report) -> None:
    ...
    missing_allowlist = [
        asset for asset in ("assistant.html", "assistant.css", "assistant.js")
        if f'"{asset}"' not in app_source
    ]
    if missing_allowlist:
        report.fail("Flask 静态资源白名单缺少：" + "、".join(missing_allowlist))
    if "connect-src 'self'" not in app_source:
        report.fail("CSP 未把前端连接限制为同源")
