...
with sync_playwright() as playwright:
    ...
    expect(page.get_by_role("link", name="影像作品")).to_be_visible()

    page.goto(f"{base_url}/film.html", wait_until="networkidle")
    expect(page.locator("h1")).to_contain_text("从平台化到智能化")
    expect(page.locator("#caseFilm")).to_have_attribute("poster", "./assets/films/nanjing-south-competition-poster.webp?v=20260907-4")
    expect(page.locator("#caseFilm source")).to_have_attribute("type", "video/mp4")
    expect(page.get_by_role("button", name="女声版")).to_have_attribute("aria-pressed", "true")
    expect(page.get_by_role("button", name="男声版")).to_have_attribute("aria-pressed", "false")
    expect(page.locator("#filmPlay")).to_be_visible()
    page.locator("#caseFilm").evaluate("video => { video.muted = true; }")
    page.locator("#filmPlay").click()
    # The production CSP intentionally disallows unsafe-eval, so poll from
    # Python while allowing slower machines enough decoder startup time.
    film_started = False
    for _ in range(80):
        film_started = page.locator("#caseFilm").evaluate(
            "video => !video.paused && video.currentTime > 0"
        )
        if film_started:
            break
        page.wait_for_timeout(250)
    expect(page.locator("#filmFrame")).to_have_class("film-frame is-playing")
    assert film_started
    page.locator("#caseFilm").evaluate("video => video.pause()")
    ...
    page.locator("#origins").scroll_into_view_if_needed()
    stage_box = page.locator(".chronicle-stage").bounding_box()
    list_box = page.locator(".chronicle-list").bounding_box()
    assert stage_box is not None and list_box is not None
    assert abs(stage_box["y"] - list_box["y"]) < 2
    assert abs(stage_box["height"] - list_box["height"]) < 2
    ...
    expect(page.locator(".side-nav a")).to_have_count(8)
    expect(page.locator("h1")).to_contain_text("从平台化到智能化：")
    expect(page.locator("h1")).to_contain_text("政企共创平台企业何以")
    expect(page.locator("h1")).to_contain_text("化解行政分割难题？")
    ...
    expect(page.locator("#agentExplainerVideo")).to_have_attribute("poster", "assets/films/hubcoord-agent-60s-poster.webp")
    expect(page.locator("#agentExplainerVideo source")).to_have_attribute("type", "video/mp4")
    assert page.locator("#agentFaq details").count() == 8
    assert page.locator("#agentFaq details").first.evaluate("item => item.open")
    page.locator("#agentFaq details summary").nth(1).focus()
    page.keyboard.press("Enter")
    assert page.locator("#agentFaq details").nth(1).evaluate("item => item.open")
    page.locator("#agentExplainerVideo").evaluate("video => { video.muted = true; return video.play(); }")
    page.wait_for_timeout(1200)
    assert page.locator("#agentExplainerVideo").evaluate("video => video.duration >= 60 && video.duration < 61 && video.currentTime > 0")
    page.locator("#agentExplainerVideo").evaluate("video => video.pause()")
    ...
    expect(page.locator("#presentationProgress")).to_have_text("答辩 1 / 9")
    page.screenshot(path="hubcoord-presentation-final.png")
    page.locator("#presentationNext").click()
    expect(page.locator("#presentationProgress")).to_have_text("答辩 2 / 9")
    ...
    expect(page.locator("#assistantAnswer")).to_contain_text("不可以", timeout=15_000)
    expect(page.locator("#assistantAnswer")).to_contain_text("AR-SC01-03", timeout=15_000)
    ...
    page.locator("input[value='verify_evidence']").check()
